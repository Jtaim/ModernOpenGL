...
int main()
{
    ...
    Transform trans1{{0.5f, -0.5f, 0.0f}, {}, {0.5, 0.5, 0.5}};
    Transform trans2{{-0.5f, 0.5f, 0.0f}};
    ...
    while(!window.IsClosed()){
        ...
        glBindVertexArray(VAO);
        glUniformMatrix4fv(transformLoc, 1, GL_FALSE, glm::value_ptr(trans1.GetModel()));
        trans1.SetRotation({0.0f, 0.0f, static_cast<float>(glfwGetTime())});
        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);

        auto scale{std::sinf(static_cast<float>(glfwGetTime()))};
        glUniformMatrix4fv(transformLoc, 1, GL_FALSE, glm::value_ptr(trans2.GetModel()));
        trans2.SetScale({scale, scale, 1.0f});
        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
