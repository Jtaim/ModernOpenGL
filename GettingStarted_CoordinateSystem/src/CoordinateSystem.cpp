...
int main()
{
    ...
    Shader shader{"./shaders/coordinate.vert", "./shaders/texture_combined.frag"};
    ...
    auto viewLoc{shader.GetUniformLocation("view")};
    auto projectionLoc{shader.GetUniformLocation("projection")};

    Transform model;
    model.SetRotation({glm::radians(-55.0f), 0.0f, 0.0f});
    Transform view;
    view.SetPosition({0.0f, 0.0f, -10.0f});
    ...
    while(!window.IsClosed()){
        ...
        glUniformMatrix4fv(viewLoc, 1, GL_FALSE, glm::value_ptr(view.GetModel()));
